...
def select_top_k_pairs_gpu(energy: Tensor, k: int, max_features: Optional[int] = None, pairing_strategy: str = "consecutive") -> Tensor:
    ...
    # Keep a larger oversampled pool so the downstream greedy pass can still
    # find enough disjoint pairs when the highest-scoring candidates overlap.
    ...
    # Greedy selection must enforce pairwise disjointness. The previous batched
    # implementation could emit overlapping pairs because availability was
    # checked before updating `used_mask` within the batch.
