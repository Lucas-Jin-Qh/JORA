#!/usr/bin/env python3
"""Run the phase-2 JORA follow-up.

By default this launcher opportunistically uses GPUs 0/1/2, prioritizing free
memory over low utilization so it can share cards with other users when the
remaining VRAM is still sufficient.

Phase 2 extends the completed single-seed formal rollout with:

- an appendix-style JORA core sweep on seed 42
- a three-way JORA shape probe on seed 42
- promotion of the best JORA shape
- three-seed anchors against LoRA-r1 and LoRA-r2
...
DEFAULT_WORKDIR = REPO_ROOT / "formal_runs" / "three_gpu_bf16_phase2"
...
launcher = _load_module("single_gpu_bf16_plan_phase2", LAUNCHER_PATH)
rollout = _load_module("run_three_gpu_rollout_phase2", ROLLOUT_PATH)
...
train_spec = rollout.train_spec
update_state_phase = rollout.update_state_phase
wait_for_gpus = rollout.wait_for_gpus
...
def build_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=DEFAULT_PHASE2_GPUS,
        help="GPU ids eligible for phase-2 scheduling. Defaults to opportunistic use of GPUs 0/1/2.",
    )
    ...
    parser.add_argument(
        ...
        help="Minimum free VRAM (MiB) required before launching a phase-2 job on a GPU.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--core-sweep-steps", type=int, default=500)
    parser.add_argument("--core-sweep-mmlu-limit", type=int, default=200)
    parser.add_argument("--shape-probe-steps", type=int, default=500)
...
def checkpoint_interval(max_steps: int, segments: int) -> int:
    return max(1, max_steps // max(1, segments))


def logging_interval(max_steps: int) -> int:
    return max(10, max_steps // 20)
...
def build_core_sweep_specs(best_theta: float, best_core: float, max_steps: int) -> list[Any]:
    base_jora = replace(
        launcher._base_named_specs()["m2b_jora"],
        seed=CLAIM_SEEDS[0],
        max_steps=max_steps,
        logging_steps=logging_interval(max_steps),
        save_strategy="steps",
        save_steps=checkpoint_interval(max_steps, 2),
        save_total_limit=1,
    ...
    for candidate in CORE_CANDIDATES:
        specs.append(
            replace(
                ...
                description=f"Phase-2 core sweep for {candidate.tag} on seed {base_jora.seed}.",
    ...
    baseline_lora_r1 = replace(
        launcher._base_named_specs()["m2b_lora_r1"],
        name="core_sweep_lora_r1_seed42",
        description="Phase-2 core sweep baseline LoRA-r1 on seed 42.",
    ...
    baseline_lora_r2 = replace(
        launcher._base_named_specs()["m2b_lora_r2"],
        name="core_sweep_lora_r2_seed42",
        description="Phase-2 core sweep baseline LoRA-r2 on seed 42.",
...
def build_shape_probe_specs(best_theta: float, best_core: float, max_steps: int) -> list[Any]:
    base = replace(
        launcher._base_named_specs()["m2b_jora"],
        seed=CLAIM_SEEDS[0],
        max_steps=max_steps,
    ...
    for candidate in SHAPE_CANDIDATES:
        specs.append(
            replace(
                ...
                description=f"Phase-2 JORA shape probe for {candidate.tag} on seed {base.seed}.",
...
def build_anchor_specs(selected_shape: ShapeCandidate, best_theta: float, best_core: float, seed: int, max_steps: int) -> list[Any]:
    save_steps = checkpoint_interval(max_steps, 4)
    jora = replace(
        launcher._base_named_specs()["m2b_jora"],
        name=f"anchor_{selected_shape.family}_seed{seed}",
        description=f"Phase-2 three-seed anchor for {selected_shape.tag} on seed {seed}.",
    ...
    lora_r1 = replace(
        ...
        description=f"Phase-2 three-seed LoRA-r1 anchor on seed {seed}.",
    ...
    lora_r2 = replace(
        ...
        description=f"Phase-2 three-seed LoRA-r2 anchor on seed {seed}.",
        ...
        if pending or running:
            if not made_progress:
                if not args.dry_run:
                    states = query_gpu_states()
                    print(
                        "Waiting for an eligible GPU: " + format_gpu_summary(gpu_ids, states),
                        flush=True,
                    )
...
def run_core_sweep(args: argparse.Namespace, state: dict[str, Any], best_theta: float, best_core: float) -> dict[str, Any]:
    specs = build_core_sweep_specs(best_theta, best_core, args.core_sweep_steps)
...
def run_shape_probe(args: argparse.Namespace, state: dict[str, Any], best_theta: float, best_core: float) -> ShapeCandidate:
    specs = build_shape_probe_specs(best_theta, best_core, args.shape_probe_steps)
    runs = run_parallel_specs_with_mmlu_eval(
        specs,
        args.gpus,
        args,
    ...
    while pending or running:
        ...
        while pending and launchable_gpus:
            ...
            if benchmark_result_complete(partial_output, ["arc_challenge", "gsm8k"]):
                merge_full_benchmarks(output_dir / ANCHOR_MMLU_OUTPUT_NAME, partial_output, merged_output)
        ...
        for gpu_id, (spec_name, output_dir, process) in list(running.items()):
            ...
            merged_output = output_dir / FULL_MERGED_OUTPUT_NAME
            merge_full_benchmarks(output_dir / ANCHOR_MMLU_OUTPUT_NAME, output_dir / FULL_PARTIAL_OUTPUT_NAME, merged_output)
            outputs[spec_name] = str(merged_output)
            completed_gpus.append(gpu_id)
            made_progress = True
        ...
        if pending or running:
            if not made_progress:
                states = query_gpu_states()
                print(
                    "Waiting for an eligible GPU for full evals: " + format_gpu_summary(gpu_ids, states),
                    flush=True,
                )
                time.sleep(min(args.poll_interval, 10))
...
def run_anchor_rounds(
    ...
    for seed in CLAIM_SEEDS:
        specs = build_anchor_specs(selected_shape, best_theta, best_core, seed, args.anchor_steps)
        runs = run_parallel_specs_with_mmlu_eval(specs, args.gpus, args, output_name=ANCHOR_MMLU_OUTPUT_NAME)
        full_eval_outputs = run_parallel_full_evals(runs, args.gpus, args)
        for spec_name, output_path in full_eval_outputs.items():
            runs[spec_name]["benchmarks_output"] = output_path
        phase_payload["rounds"][f"seed{seed}"] = {
            "seed": seed,
            "runs": runs,
        }
...
def summarize_core_sweep(core_payload: dict[str, Any] | None) -> dict[str, Any] | None:
    if not core_payload:
        return None
    return {
        "leaderboard": core_payload.get("leaderboard", []),
    }
...
def write_phase2_summary(
    ...
    summary = aggregate_anchor_results(anchor_payload)
    core_summary = summarize_core_sweep(core_payload)
    if core_summary is not None:
        summary["core_sweep"] = core_summary
...
def main() -> None:
    ...
    state: dict[str, Any] = {
        "settings": {
            ...
            "core_sweep_steps": args.core_sweep_steps,
            "core_sweep_mmlu_limit": args.core_sweep_mmlu_limit,
            "shape_probe_steps": args.shape_probe_steps,
    ...
    write_phase2_summary(args, state, anchor_payload, core_payload)
